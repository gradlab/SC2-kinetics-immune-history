...
filename_base <- paste0("outputs/base_age_models")
...
formulas <- list(
    ...
    bf(low_ct1 ~ AgeGroup + LineageBroad_VaccStatus + s(DaysSinceDetection) + s(DaysSinceDetection,by=AgeGroup) + s(DaysSinceDetection,by=LineageBroad_VaccStatus)),
    
    bf(low_ct1 ~ LineageBroad_VaccStatus_AgeGroup + s(DaysSinceDetection) + s(DaysSinceDetection,by=LineageBroad_VaccStatus_AgeGroup))
)
names <- c("baseline_age","vaccine_and_lineage_age","vacc_lineage_age")
