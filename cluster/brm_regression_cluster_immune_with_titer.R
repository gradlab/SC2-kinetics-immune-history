...
dat_subset_use <- dat_subset_use %>% filter(!is.na(TiterGroup), 
                                            !is.na(LineageBroad_BoostTiterGroup),
                                            !is.na(BoostTiterGroup))

filename_base <- paste0("outputs/immune_models_titer")
...
names <- c("baseline","vaccine","cumulative_exposures","days_since_exposure",
           ...
           "boost_titer","boost_titer_lineage","boost_titer_lineage_age"
           )
...
if(rerun_stan){
    ...
    save(fit, file=paste0("outputs/immune_models_titer/",name,"_",use_data,".RData"))
} else {
    load(paste0("outputs/immune_models_titer/",name,"_",use_data,".RData"))
}
## Assess performance
...
save(kfold_est, file=paste0("outputs/immune_models_titer/",name,"_",use_data,"_kfolds",".RData"))
